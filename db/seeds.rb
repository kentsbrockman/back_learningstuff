...
# COURSES
Course.create(title: 'Apprenez à créer votre site web avec HTML5 et CSS3', description: "Vous souhaitez créer vos propres sites web ? Vous êtes au bon endroit ! Dans ce cours, vous apprendrez comment utiliser HTML5 et CSS3, les deux langages de programmation à la base de tous les sites web.
...
Course.create(title: 'Découvrez le monde des start-ups', description: "L’univers et le fonctionnement des start-ups vous intéressent ? Vous avez quelques idées en tête, sans savoir comment vous lancer ? Vous aimeriez savoir où vous former ? Comment rencontrer les bonnes personnes ? Ce cours est fait pour vous !
...
Course.create(title: "Maîtrisez les fondamentaux d'Excel", description: "Le logiciel Excel fait partie des logiciels les plus utilisés dans le monde. Tous ceux qui s'en servent au quotidien savent que ce n'est pas sans raison ! Excel vous permet facilement de traiter de nombreuses données et de les mettre en forme.
...
Course.create(title: "Python n'est toujours pas un serpent !", description: "Pour créer des programmes informatiques ou construire des algorithmes destinés à l’analyse de données, vous devez maîtriser un langage de programmation.
...
Course.create(title: "Python n'est pas un serpent !", description: "Grâce à la programmation orientée objet, nous allons réaliser ensemble un monde virtuel peuplé d'humains, qui nous servira de terrain d'expérimentation pour vérifier notre théorie !
...
Course.create(title: 'Ruby on rails est tout ce dont vous avez toujours rêvé !', description: "Le monde du Web vous intéresse, mais vous n’y connaissez rien ? Vous décrochez quand vous entendez du vocabulaire geek ? Ce cours a pour objectif de vous mettre à niveau. Aucune connaissance préalable n’est requise.
...
Course.create(title: 'Débutez la programmation avec Java', description: "Pour créer des programmes informatiques ou concevoir l'application de vos rêves, vous devez savoir coder dans un langage de programmation. Les programmes s'appuient sur des données et sur la logique pour fonctionner. Pour cela, un programmeur (vous !) doit dire à l'ordinateur ce qu'il doit faire et comment il doit le faire.")
Course.create(title: "Entraine-toi à l'algorithmie", description: "Comment un GPS trouve-t-il l’itinéraire qui nous fera esquiver les bouchons ? Comment Google nous présente-t-il toujours la page que nous cherchons, ou Spotify sait-il avant nous la prochaine musique qui nous fera vibrer ?
...
Course.create(title: 'Écrivez du JavaScript pour le web', description:"Dans ce cours, vous apprendrez à manipuler le DOM pour récupérer des éléments d'une page et de modifier leur contenu, leurs classes, leurs styles etc. Cela rendra vos pages web plus dynamiques.
...
# LEARNING PATHS
business =
  LearningPath.create(title: 'Créez votre propre entreprise', price_in_cents: 50_000)
business.courses << [
...
business.categories << [Category.find_by(title: 'Business'), Category.find_by(title: 'Management')]
...
code.courses << [
  ...
  Course.find_by(title: 'Utilisez Git et GitHub pour vos projets de développement')
]
...
pythonadv =
  LearningPath.create(title: "Python n'est toujours pas un serpent !", price_in_cents: 30_000)
pythonadv.courses << Course.find_by(title: "Python n'est toujours pas un serpent !")
pythonadv.categories << [
  Category.find_by(title: 'Coding'),
  Category.find_by(title: 'Python')
]

ruby = LearningPath.create(title: 'Ruby on rails est tout ce dont vous avez toujours rêvé !', price_in_cents: 30_000)
ruby.courses <<
  Course.find_by(title: 'Ruby on rails est tout ce dont vous avez toujours rêvé !')
ruby.categories << [
  Category.find_by(title: 'Coding'),
...
jsnoob =
  LearningPath.create(title: 'Écrivez du JavaScript pour le web', price_in_cents: 5000)
...
codewars =
  LearningPath.create(title: "Entraine-toi à l'algorithmie", price_in_cents: 5000)
codewars.courses <<
  Course.find_by(title: "Entraine-toi à l'algorithmie")
codewars.categories << [Category.find_by(title: 'Coding')]

jsadv = LearningPath.create(title: 'Apprenez à programmer avec JavaScript', price_in_cents: 5000)
...
LearningPath.all.each { |learning_path| learning_path.update(description: Faker::Lorem.paragraph(sentence_count: 4))}
...
Course.where.missing(:chapters).each do |course|
...
student.subscriptions.create(
  learning_path: LearningPath.find_by(title: 'Apprenez à créer votre site web avec HTML5 et CSS3')
)
student
  .subscriptions
  .last
