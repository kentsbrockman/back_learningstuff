class Subscription < ApplicationRecord
  ...
  def current_lesson
    if self.learning_path.courses.first.number_lessons == 0
      return nil
    elsif !self.learning_path.courses.first.progress_states.find_by(user:self.user)&.progress_lessons
      self.learning_path.courses.first.first_chapter.first_lesson
    else
      self.learning_path.courses.first.progress_states.find_by(user:self.user).progress_lessons.last&.lesson&.next_lesson
    end
  end
end
